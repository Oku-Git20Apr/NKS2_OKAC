...
cout<<"before file open"<<endl;
  ...
  TChain *tree = new TChain("TTreeForNKS2SimEvent");
  tree->Add("1017.root"); 
...
//  tree->Draw("fNKS2SimIHHit.mSegment");
  TH1D *segment;
  segment = new TH1D("segment","segment",10,0.,10.);
  tree->Project("segment","fNKS2SimIHHit.mSegment");
  double Max;
  Max = segment->GetBinContent(segment->GetMaximumBin());
  cout << "Max = " << Max << endl;
//  segment->GetMaximum(); 
  segment->Draw();
