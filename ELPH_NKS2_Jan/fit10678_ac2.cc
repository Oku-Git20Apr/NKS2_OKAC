...
void draw(){
  ...
  double lambda = 10;    //average of #photon
  double res = 0.2;      //energy resolution factor
  int roop = 400;        //menseki

  TChain *tree = new TChain("tree");
  tree->Add("10676.root");

  //                     ch1    ch2
  float pedestal[2] = { 163, 82 };  
  float peak_pe[2] = { 329, 206 };

    TH1D *ch1_ohhldadc;     //ohhldadc[7]
    ch1_ohhldadc = new TH1D("ch1_ohhldadc","ch1_ohhldadc",1000,0,2000);  //ohhldadc channel
    ...
    tree->Project("ch1_ohhldadc","ohhldadc[7]","","");


    TH1D *ch2_ohhldadc;     //ohhldadc[8]
    ch2_ohhldadc = new TH1D("ch2_ohhldadc","ch2_ohhldadc",1000,0,2000);  //ohhldadc channel
    ...
    tree->Project("ch2_ohhldadc","ohhldadc[8]","","");
  ...
  sprintf( condi, "(ohhldadc[7]-%f)/%f+(ohhldadc[8]-%f)/%f",pedestal[0], peak_pe[0]-pedestal[0], pedestal[1], peak_pe[1]-pedestal[1] );


    tree->Project("Total", condi, "ohhldtdc[7]>0&&ohhldtdc[8]>0&&ohhldadc[7]>0&&ohhldadc[8]>0&&abs(ohhldadc[3]-750)<250&&abs(ohhldadc[6]-2250)<250");

 TF1 *f1 = new TF1("f1",F_Pois,0,10,3);
  f1->SetNpx(500);
  f1->SetParameters(lambda,res,roop);
  Total->Fit(f1,"0","",0,5);    //fitting
