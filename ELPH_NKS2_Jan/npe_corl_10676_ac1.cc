...
void draw(){
  ...
  //                     ch1    ch2
   ...
   float peak_pe[2] = { 175.8, 143.0 };
    ...
    tree->Project("ch1_qdc","ohhldadc[4]","","");

  char condi[1000];
  sprintf( condi, "(ohhldadc[4]-%f)/%f", pedestal[0], peak_pe[0]-pedestal[0] ); 

    TH1D *ch2_qdc;     //ohhldadc[5]
    ...
    tree->Project("ch2_qdc","ohhldadc[5]","","");


  char condi2[1000];

  sprintf( condi2, "(ohhldadc[5]-%f)/%f", pedestal[1], peak_pe[1]-pedestal[1] );

    char condi3[1000];
    sprintf( condi3, "%s:%s", condi, condi2 );
    
    TH2D *ch1vsch2;
    ch1vsch2 = new TH2D("ch1_pe_ch2_pe","ch1_pe vs. ch2_pe",45,0.,15.,45,0.,15.);
