...
void draw(){
  ...
  double lambda = 12;    //average of #photon
  ...
  //                     ch1    ch2
  float pedestal[2] = { 160, 100 };  
  float peak_pe[2] = { 220, 130 };
  ...
  sprintf( condi, "(ohhldadc[7]-%f)/%f+(ohhldadc[8]-%f)/%f+10",pedestal[0], peak_pe[0]-pedestal[0], pedestal[1], peak_pe[1]-pedestal[1] );


    tree->Project("Total", condi, "ohhldtdc[7]>0&&ohhldtdc[8]>0&&ohhldadc[7]>0&&ohhldadc[8]>0&&ohhldtdc[3]-ohhldtdc[6]>-100&&ohhldtdc[3]-ohhldtdc[6]<100");

 TF1 *f1 = new TF1("f1",F_Pois,0,30,3);
  f1->SetNpx(500);
  f1->SetParameters(lambda,res,roop);
  Total->Fit(f1,"0","",10,15);    //fitting
