...
//    tree->Project("Total", condi, "qdc[0]>600&&qdc[3]>1000&&tdc[1]>0&&tdc[2]>0&&qdc[1]>0&&qdc[2]>0");
 ...
 //TF1 *f1 = new TF1("f1",F_Pois,3,13,3);
  TF1 *f1 = new TF1("f1",F_Voigt,3,13,4);
  f1->SetNpx(500);
  f1->SetParameters(lambda,res,roop);
  Total->Fit(f1,"Rq0","0",3,13);    //fitting
  ...
  c1->cd(1)->SetMargin(0.15,0.15,0.15,0.15);
    ...
    Total->Draw("");
    //f1->Draw("same");
	cout<<Total->Integral(Total->FindBin(3),Total->FindBin(13))<<endl;
    cout<<"findbin(3)="<<Total->FindBin(3)<<endl;
    cout<<"this should be 10??? ->"<<Total->FindBin(13)-Total->FindBin(3)<<endl;
	cout<<f1->Integral(3,13)<<endl;
    cout<<"par0="<<f1->GetParameter(0);
