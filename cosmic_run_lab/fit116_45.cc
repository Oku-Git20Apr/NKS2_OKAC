...
void draw(){
  ...
  tree->Add("r0000116.root");

  //                     ch4    ch5
  float pedestal[2] = { 166.629, 194.237 };  
  float peak_pe[2] = { 228.258, 246.405 };

    TH1D *ch4_qdc;     //qdc[4]
    ch4_qdc = new TH1D("ch4_qdc","ch4_qdc",1000,0,2000);  //qdc channel
    
    TH1D *ch4_pe;      //number of photoelectron
    ch4_pe = new TH1D("ch4_pe","ch4_pe",175,-5,30);
    SetTH1(ch4_pe,"ch4_pe","Number of photoelectron","Counts");
    tree->Project("ch4_qdc","qdc[4]","","");

//  char condi[1000];
//
//  sprintf( condi, "(qdc[1]-%f)/%f", pedestal[0], peak_pe[0]-pedestal[0] );   //qdc channel -> number of photoelectron
//    tree->Project( "ch4_pe", condi, "tdc[1]>0&&qdc[1]>0&&tdc[4]+1.115*tdc[5]>1020&&tdc[4]+1.115*tdc[5]<1070" );

    TH1D *ch5_qdc;     //qdc[5]
    ch5_qdc = new TH1D("ch5_qdc","ch5_qdc",1000,0,2000);  //qdc channel
    
    TH1D *ch5_pe;      //number of photoelectron
    ch5_pe = new TH1D("ch5_pe","ch5_pe",175,-5,30);
    SetTH1(ch5_pe,"ch5_pe","Number of photoelectron","Counts");
    tree->Project("ch5_qdc","qdc[5]","","");

    TH1D *Total;    //ch4+ch5(NPE)
    Total = new TH1D("Total","Total", 175,-5,30);
    SetTH1(Total,"Total","Number of photoelectron","Counts");

  char condi[1000];

  sprintf( condi, "(qdc[4]-%f)/%f+(qdc[5]-%f)/%f",pedestal[0], peak_pe[0]-pedestal[0], pedestal[1], peak_pe[1]-pedestal[1] );   //qdc channel -> number of photoelectron
//    tree->Project( "ch5_pe", condi2, "tdc[2]>0&&qdc[2]>0&&tdc[4]+1.115*tdc[5]>1020&&tdc[4]+1.115*tdc[5]<1070" );


    tree->Project("Total", condi, "tdc[4]>0&&tdc[5]>0&&qdc[4]>0&&qdc[5]>0&&qdc[0]>500&&qdc[0]<1000&&qdc[3]>500&&qdc[3]<1000");
//    tree->Project("Total", condi, "qdc[0]>600&&qdc[3]>1000&&tdc[1]>0&&tdc[2]>0&&qdc[1]>0&&qdc[2]>0&&tdc[4]+1.115*tdc[5]>1020&&tdc[4]+1.115*tdc[5]<1070&&tdc[4]-tdc[5]<-180&&tdc[4]-tdc[5]>-240");
 ...
 TF1 *f1 = new TF1("f1",F_Pois,5,15,3);
  f1->SetNpx(500);
  f1->SetParameters(lambda,res,roop);
//  Total->Fit(f1,"0","",0,20);    //fitting
